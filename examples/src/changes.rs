use unc_primitives::{types::BlockReference, views::StateChangesRequestView};
use serde_json::json;
...
async fn main() -> anyhow::Result<()> {
    ...
    let block_ref = {
        let hash = unc_primitives::hash::CryptoHash(outcome.outcome().block_hash.0);
        BlockReference::BlockId(unc_primitives::types::BlockId::Hash(hash))
    };
