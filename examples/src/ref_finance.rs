...
async fn create_ref(owner: &Account, worker: &Worker<Sandbox>) -> anyhow::Result<Contract> {
    ...
    owner
        ...
        .args_json(json!({}))
        .deposit(UncToken::from_milliunc(30))
        .transact()
        .await?
        .into_result()?;
...
/// Pull down the WUnc contract from mainnet and initialize it with our own metadata.
async fn create_wunc(owner: &Account, worker: &Worker<Sandbox>) -> anyhow::Result<Contract> {
    let mainnet = unc_workspaces::mainnet_archival().await?;
    ...
    owner
        .call(wunc.id(), "storage_deposit")
        .args_json(json!({}))
        .deposit(UncToken::from_milliunc(8))
        .transact()
        .await?
        .into_result()?;

    owner
        .call(wunc.id(), "unc_deposit")
...
async fn create_pool_with_liquidity(
    ...
    let pool_id: u64 = ref_finance
        ...
        .deposit(UncToken::from_milliunc(3))
        .transact()
        .await?
        .json()?;
...
async fn main() -> anyhow::Result<()> {
    ...
    let pool_id = create_pool_with_liquidity(
        ...
        &ref_finance,
        maplit::hashmap! {
            ft.id() => UncToken::from_unc(5).as_attounc(),
            wunc.id() => UncToken::from_unc(10).as_attounc(),
        },
    )
    .await?;
    println!(
    ...
    deposit_tokens(
        ...
        &ref_finance,
        maplit::hashmap! {
            ft.id() => UncToken::from_unc(100).as_attounc(),
            wunc.id() => UncToken::from_unc(100).as_attounc(),
        },
    )
    .await?;
    ...
    println!("Current WUnc deposit: {}", wunc_deposit);
    ...
    println!(
        "Expect return for trading in 1 FT token for WUnc: {}",
        expected_return
    );
    ...
    println!(
        "Actual return for trading in 1 FT token for WUnc: {}",
        actual_out
    );
    ...
    let wunc_deposit: String = ref_finance
        ...
        .await?
        .json()?;
    println!("New WUnc deposit after swap: {}", wunc_deposit);
