...
impl<'a, T> NetworkBuilder<'a, T> {
    ...
    /// node than the default one provided by unc.org. This enables certain features that
...
impl NetworkBuilder<'_, Sandbox> {
    ...
    /// about initializing and  starting our own sandbox, go to [uncd-sandbox](https://github.com/utility/sandbox).
    /// This can be either set to a known key value or to the home directory where all the chain data lives.
    /// This is the `my_home_folder` we passed into `unc-sandbox --home {my_home_folder} init`.
