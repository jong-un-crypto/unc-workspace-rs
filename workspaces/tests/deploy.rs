...
use unc_workspaces::network::{Sandbox, ValidatorKey};
use unc_workspaces::{pick_unused_port, Worker};

const NFT_WASM_FILEPATH: &str = "../examples/res/non_fungible_token.wasm";
const EXPECTED_NFT_METADATA: &str = r#"{
  "spec": "nft-1.0.0",
  "name": "Example UNC non-fungible token",
  "symbol": "EXAMPLE",
...
async fn test_manually_spawned_deploy() -> anyhow::Result<()> {
    ...
    let output = unc_sandbox_utils::init(&home_dir)?
        .wait_with_output()
        .await
        .unwrap();
    tracing::info!(target: "workspaces-test", "sandbox-init: {:?}", output);

    let mut child = unc_sandbox_utils::run(&home_dir, rpc_port, net_port)?;
